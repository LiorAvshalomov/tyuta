          ...
          <div className="grid items-stretch gap-6 lg:grid-cols-2">
            ...
            <section className="pd-intro-stagger hidden lg:block rounded-3xl p-10">
              ...
              <ul className="mt-6 space-y-3 text-sm text-black/70">
                <li className="flex items-start gap-2"><span aria-hidden>📓</span><span>המחברת שלך נשמרת ומחכה.</span></li>
                <li className="flex items-start gap-2"><span aria-hidden>🖋️</span><span>כתיבה קצרה או ארוכה — הכול הולך.</span></li>
                <li className="flex items-start gap-2"><span aria-hidden>🕊️</span><span>לפעמים שורה אחת מספיקה כדי להתחיל.</span></li>
              </ul>
            </section>

            {/* Form card */}
            <section className="pd-auth-card pd-intro rounded-3xl p-6 sm:p-8">
              {children}
            </section>
          </div>
        </div>
