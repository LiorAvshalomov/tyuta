...
export async function GET(req: Request) {
  ...
  // list suspended users ordered by suspended_at desc
  ...
  // Keep same order as mods
  users.sort((a, b) => {
    const at = new Date(a.moderation.suspended_at ?? 0).getTime()
    const bt = new Date(b.moderation.suspended_at ?? 0).getTime()
    return bt - at
  })
